...
const SearchBar: React.FC<SerachBarProps> = ({ onSearch, loading }) => {
  ...
  return (
    <div ref={searchRef} className="w-full max-w-2xl mx-auto relative">
      <form onSubmit={handleSubmit}>
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-22 pr-12 py-4 bg-black border-2 border-white/20 rounded-xl text-white placeholder-gray-400 focus:outline-none focus:border-white/40 focus:ring-2 focus:ring-white/10 transition-all duration-300 text-lg backdrop-blur-sm"
            disabled={loading}
          />
      ...
      {showSuggestions && suggestions.length > 0 && (
        <div className="absolute top-full left-0 right-0 mt-2 bg-black border-white/20 border-2 rounded-xl shadow-2xl backdrop-blur-sm z-100 overflow-hidden">
          <div className="py-2">
            {query.trim().length === 0 && (
