    ...
    <div className="min-h-screen bg-black text-white">
      <nav className="bg-black border-b-2 border-white/20 backdrop-blur-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {/* Mobile Dropdown */}
          {menuOpen && (
            <div className="lg:hidden mt-4 space-y-4 pb-4 border-t border-white/10">
              {user ? (
                <>
                  <Link
                    to="/saved"
                  ...
                  <div className="px-4 py-2 text-sm text-gray-300 flex items-center space-x-2 bg-white/5 rounded-xl border border-white/10">
                    <User className="w-4 h-4" />
                    <span>{user.name}</span>
                  </div>
                  <button
